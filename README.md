...
- Menu bar utility with a custom camera status icon.
...
Use `scripts/run-app.sh` for manual testing instead of `swift run ezshot`. macOS Screen Recording permission is tied to the launched app identity. `scripts/run-app.sh --rebuild` rebuilds and signs `.build/Ezshot.app`; plain `scripts/run-app.sh` relaunches the existing bundle without changing its signature, which avoids disturbing Screen Recording permission during repeated manual tests.
